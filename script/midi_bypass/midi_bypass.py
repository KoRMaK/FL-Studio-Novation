...
class MidiBypass:
    ...
    def on_note_on(self, eventData, launchkey_selected_channel=None):
        ...
        #print(f"[midi_bypass] note_on: lk_ch={launchkey_selected_channel} target={target_channel} note={eventData.data1} vel={eventData.data2}")
        if target_channel is None:
            #print("[midi_bypass] note_on: DROPPED (no target channel) — NOT claiming handled")
            return
    ...
    def on_note_off(self, eventData, launchkey_selected_channel=None):
        ...
        target_channel = self._get_target_channel(launchkey_selected_channel)
        if target_channel is None:
            #print(f"[midi_bypass] note_off: DROPPED (no target channel) note={eventData.data1}")
            return
